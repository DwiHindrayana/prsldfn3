...
def updated_stats(chat, queue, vol=100):
    if chat.id in callsmusic.active_chats:
        ...
        if len(que) > 0:
            ...
            stats += "sekarang memainkan : **{}**\n".format(queue[0][0])
            stats += "direquest oleh si babi : {}".format(queue[0][1].mention)
    else:
        stats = None
    return stats
...
async def ee(client, message):
    ...
    if stats:
        ...
        await message.reply("Voice chat nya idupin dulu sat")
...
async def settings(client, message):
    ...
    if stats:
        ...
        await message.reply("voice chatnya idupin dulu sat")
...
async def play(_, message: Message):
            ...
            if administrator == message.from_user.id:
                ...
                try:
                    ...
                    await USER.send_message(
                        message.chat.id, "gua budaknya prisil, gua dateng cuma untuk play lagu"
                    )
                    await lel.edit(
    ...
    try:
        ...
        await lel.edit(
            f"<i> {user.first_name} budaknya prisil gaada di sini, minta admin untuk perintahkan /play pertama kali atau add manual {user.first_name} </i>"
        )
        return
    text_links=None
    ...
    if audio:
        if round(audio.duration / 60) > DURATION_LIMIT:
            await lel.edit(
                f"❌ video lebih panjang dari {DURATION_LIMIT} minute(s) tidak diperbolehkan!"
            )
            return
        ...
        title = file_name
        thumb_name = "https://telegra.ph/file/661ab4453ed2e986a672d.png"
        ...
        query = toxt
        await lel.edit("🎵 <b>ok diproses</b>")
        ...
        try:
            ...
            await lel.edit(
                "lagunya ga ketemu, ketik yang bener tolol."
            )
            print(str(e))
            return
        try:    
            ...
            if (dur / 60) > DURATION_LIMIT:
                 await lel.edit(f"❌ video lebih panjang dari {DURATION_LIMIT} minutes tidak diperbolehkan!")
                 return
        except:
            pass        
        dlurl=url
        ...
        for i in message.command[1:]:
            query += " " + str(i)
        print(query)
        await lel.edit("🎵 **ok diproses**")
        ydl_opts = {"format": "bestaudio[ext=m4a]"}
        
        try:
          results = YoutubeSearch(query, max_results=5).to_dict()
        except:
          await lel.edit("pilih sesuatu untuk diplay")
        # Looks like hell. Aren't it?? FUCK OFF
        try:
            toxxt = "**Nih dipilih dulu lagunya**\n\n"
            j = 0
            useer=user_name
            ...
            try:
                ...
                await lel.edit(
                    "lagunya ga ketemu. kalo ngetik yang bener"
                )
                print(str(e))
                return
            try:    
    ...
    if chat_id in callsmusic.active_chats:
        ...
        await message.reply_photo(
            photo="final.png",
            caption=f"#⃣ lagu yang lu minta <b>queued</b> di posisi {position}!",
        ...
        await message.reply_photo(
            photo="final.png",
            reply_markup=keyboard,
            caption="▶️ <b>Playing</b> ni lagu yang diminta si babi {} via YouTube Music".format(
...
async def ytplay(_, message: Message):
    ...
    if message.chat.id in DISABLED_GROUPS:
        return
    lel = await message.reply("🔄 <b>ok diproses</b>")
    administrators = await get_administrators(message.chat)
    chid = message.chat.id
    ...
    try:
        ...
        await lel.edit(
            f"<i> {user.first_name} budak prisil gaada di sini, minta admin untuk perintahkan /play pertama kali atau add manual {user.first_name}</i>"
        )
        return
    await lel.edit("🔎 <b>Finding</b>")
    user_id = message.from_user.id
    ...
    try:    
        ...
        for i in range(len(dur_arr)-1, -1, -1):
            ...
            secmul *= 60
        if (dur / 60) > DURATION_LIMIT:
             await lel.edit(f"❌ Video video lebih panjang dari {DURATION_LIMIT} minutes tidak diperbolehkan!")
             return
    except:
        pass    
    dlurl=url
    ...
    if chat_id in callsmusic.active_chats:
        ...
        await message.reply_photo(
            photo="final.png",
            reply_markup=keyboard,
            caption="▶️ <b>Playing</b> ini lagu yang diminta si babi {} via YouTube Music".format(
                message.from_user.mention()
            ),
        )
        os.remove("final.png")
...
async def lol_cb(b, cb):
    ...
    if cb.from_user.id != useer_id:
        await cb.answer("lu bukan orang yang request cok. elah cape gua!", show_alert=True)
        return
    await cb.message.edit("Ditunggu ya... lagi memulai")
    x=int(x)
    try:
    ...
    if chat_id in callsmusic.active_chats:
        ...
        await b.send_photo(chat_id,
            photo="final.png",
            reply_markup=keyboard,
            caption=f"▶️ <b>Playing</b> ini lagu yang diminta si babi {r_by.mention} via YouTube Music",
        )
